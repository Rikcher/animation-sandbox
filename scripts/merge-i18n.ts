...
for (const folder of i18nFolders) {
  for (const locale of LOCALES) {
    ...
    // biome-ignore lint/security/noGlobalEval: <i18n setup uses .ts files, so it will do just fine>
    const obj = eval(`(${match[1]})`) as Record<string, string>;
    for (const key in obj) {
      if (
        mergedTranslations[locale][key] &&
        mergedTranslations[locale][key] !== obj[key]
      ) {
        console.warn(
          `Duplicate key ${key} in folder ${path.relative(ROOT, folder)}, overriding`,
        );
      }
      mergedTranslations[locale][key] = obj[key];
    }
